...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-[var(--color-border)] last:border-0">
      <span className="w-36 flex-shrink-0 text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wide pt-0.5">{label}</span>
...
function RoleBadge({ role }: { role: 'student' | 'teacher' }) {
  return role === 'teacher' ? (
    ...
    <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--color-primary)]/30 bg-[var(--color-primary)]/10 px-3 py-1 text-xs font-semibold text-[var(--color-primary)]">
      <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14a7 7 0 00-7 7h14a7 7 0 00-7-7zm0-3a3 3 0 100-6 3 3 0 000 6z" /></svg>
      Étudiant
    </span>
  )
}
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-white)] p-6">
        <div className="flex items-center gap-5">
          <div className="flex h-18 w-18 flex-shrink-0 items-center justify-center rounded-full bg-[var(--color-primary)] text-white font-bold text-xl select-none" style={{ height: 72, width: 72 }}>{initials}</div>
        ...
        <div className="mt-6 divide-y divide-[var(--color-border)]">
          ...
          <InfoRow label="Compte" value={<span className="inline-flex items-center gap-1.5 text-xs font-medium"><span className="h-2 w-2 rounded-full bg-green-500" /> Actif</span>} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-white)] px-4 py-3">
        <svg className="h-4 w-4 text-[var(--color-text-secondary)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <p className="text-xs text-[var(--color-text-secondary)]">Ces informations sont en lecture seule. Pour modifier vos données, contactez votre administrateur.</p>
      </div>
    </div>
  )
}
...
function TeacherProfile({ user }: { user: NonNullable<ReturnType<typeof useAuth>['user']> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-white)] p-6">
        <div className="flex items-center gap-5">
          <div className="flex h-18 w-18 flex-shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white font-bold text-xl select-none" style={{ height: 72, width: 72 }}>{initials}</div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap mb-1">
              <h1 className="text-xl font-bold text-[var(--color-text)] truncate">{displayName}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard label="Devoirs" value={tasks.length} sub="assignés" />
          ...
          {mockGroups.map(g => {
            ...
            return (
              <div key={g.id} className="flex items-center gap-3 rounded-xl border border-[var(--color-border)] px-4 py-3">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold">{g.name.charAt(0)}</div>
                <div className="flex-1 min-w-0">
